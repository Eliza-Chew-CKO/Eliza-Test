...
export async function mapUsers(
  ...
  for (const row of allRows) {
    ...
    if (!dryRun) {
      const optStr = (v: unknown) => String(v ?? '').trim() || undefined;
      await prisma.user.upsert({
        ...
        create: {
          ...
          department: optStr(row['CK Department'] ?? row['Department']),
          role: role as any,
          salesRegion: optStr(row['Sales Region'] ?? row['Region']),
          country: optStr(row['Country (text only)'] ?? row['Country']),
          seniority: optStr(row['Seniority']),
          pod: optStr(row['Pods']),
          managerName: optStr(row['Manager: Full Name']),
